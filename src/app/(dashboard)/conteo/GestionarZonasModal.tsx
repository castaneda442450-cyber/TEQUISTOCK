...
// ─── Formulario de zona (crear/editar) ──────────────────────────────────────
...
function TabMisZonas({ zonas, initialEditZonaId }: { zonas: Zona[]; initialEditZonaId?: string }) {
  const [formTarget, setFormTarget] = useState<"new" | Zona | null>(
    () => zonas.find((z) => z.id === initialEditZonaId) ?? null,
  );
...
function TabAsignar({
  ...
  function handleGuardar() {
    ...
    startTransition(async () => {
      ...
      if (res.error) sileo.error({ title: res.error });
      else sileo.success({ title: "Asignación guardada" });
    });
  }
...
export default function GestionarZonasModal({ zonas, productos, initialTab, initialZonaId, initialEditZonaId, onClose }: Props) {
    ...
    >
      ...
      >
        ...
        <div style={{ padding: "18px 24px", overflowY: "auto", flex: 1 }}>
          {tab === "mis-zonas" ? (
            <TabMisZonas zonas={zonas} initialEditZonaId={initialEditZonaId} />
          ) : (
            <TabAsignar zonas={zonas} productos={productos} initialZonaId={initialZonaId} />
          )}
        </div>
      </div>
    </div>
